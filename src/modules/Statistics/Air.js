...
function Temp() {
    ...
    const options = {
        ...
        plugins: {
            title: {
                display: true,
                text: 'Đồ thị cảm biến độ ẩm không khí',
            },
            annotation: {
                annotations: {
                    line1: {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await axios({
                method: "get",
                url: `http://localhost:3001/device/getDataEquipById/v2`,
    ...
    const data = {
        datasets: [
            {
                label: 'Cảm biến độ ẩm không khí',
                data: dataChart.map((x) => ([
                    new Date(x[0]).toLocaleString(),
                    x[1]
                ])),
                borderColor: 'rgb(50, 168, 194)',
                backgroundColor: 'rgba(50, 168, 194, 0.5)',
            },
        ],
    };
    const columns = [
